...
describe("GoogleLoggingApi", () => {
    ...
    it("builds the expected scope filters", async () => {
        ...
        const scenarios: Array<{ scope: ErrorLogScope; expected: string; severity: string }> = [
            { scope: "blaise", expected: "jsonPayload.source_name=\"Blaise\"", severity: "severity>=ERROR" },
            { scope: "non-blaise", expected: "NOT jsonPayload.source_name=\"Blaise\"", severity: "severity>=ERROR" },
            { scope: "nisra", expected: "resource.labels.service_name=~\"^nisra-.*\"", severity: "severity>=DEFAULT" },
            { scope: "bts", expected: "resource.labels.service_name=~\"^bts-.*\"", severity: "severity>=DEFAULT" },
            { scope: "restapi", expected: "jsonPayload.message=~\"^RESTAPI: .*\" AND jsonPayload.computer_name=~\"^restapi-.*\"", severity: "severity>=DEFAULT" }
        ];

        for (const scenario of scenarios) {
            await api.getErrorLogsLast24Hours(scenario.scope);
            const request = getEntriesMock.mock.calls.at(-1)?.[0] as { filter: string };
    ...
    it("paginates and normalizes error log entries", async () => {
        vi.spyOn(Date, "now").mockReturnValue(new Date("2026-06-11T12:00:00.000Z").getTime());
        ...
        getEntriesMock
            .mockResolvedValueOnce([
                [
                    { metadata: { timestamp: new Date("2026-06-11T10:00:00.000Z") }, data: { message: "Log 1" } },
                    { metadata: { timestamp: "2026-06-11T09:00:00.000Z" }, data: { foo: "bar" } }
                ],
                { pageToken: "next-1" }
            ])
            .mockResolvedValueOnce([
                [
                    ...
                    { metadata: { timestamp: { seconds: Number.NaN } }, data: "Fallback timestamp" },
                    { metadata: { timestamp: "2026-06-11T08:00:00.000Z" }, data: "Raw string message" },
        ...
        expect(result[4]).toEqual({ timestamp: "2026-06-11T08:00:00.000Z", log: "Raw string message" });
        expect(result[5]).toEqual({ timestamp: "2026-06-11T07:00:00.000Z", log: "42" });
    ...
    it("aggregates CAWI successful login counts for the last 24 hours", async () => {
        vi.spyOn(Date, "now").mockReturnValue(new Date("2026-06-11T12:30:00.000Z").getTime());
        ...
        expect(result.find((point) => point.timestamp === "2026-06-11T12:00:00.000Z")?.count).toEqual(2);
        expect(result.find((point) => point.timestamp === "2026-06-11T11:00:00.000Z")?.count).toEqual(0);

        const request = getEntriesMock.mock.calls[0][0] as { filter: string; orderBy: string; pageSize: number };
        expect(request.filter).toContain("resource.labels.module_id=\"cawi-portal\"");
        expect(request.filter).toContain("jsonPayload.message=~\"^Successful auth with questionnaire: .*\"");
    ...
    it("constructs Logging with the provided project id", () => {
        ...
        expect(loggingConstructorMock).toHaveBeenCalledWith({ projectId: "project-abc" });
    });
