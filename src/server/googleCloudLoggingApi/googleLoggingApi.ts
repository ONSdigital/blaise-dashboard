...
export class GoogleLoggingApi implements GoogleErrorLogging {
    ...
    private readonly errorLogsCache = new Map<ErrorLogScope, { expiresAt: number; data: GcpErrorLog[] }>();
    private cawiCountsCache: { expiresAt: number; data: CawiLoginCount[] } | null = null;
    ...
    async getCawiSuccessfulLoginCountsLast24Hours(): Promise<CawiLoginCount[]> {
        ...
        const counts = Array.from(countsByHour.entries()).map(([timestamp, count]) => ({
            timestamp,
            count
        }));
