...
describe("GoogleMonitoringApi", () => {
    ...
    it("returns uptime check configs", async () => {
        listUptimeCheckConfigsMock.mockResolvedValue([[{ displayName: "uptime-check" }]]);
    ...
    it("rethrows uptime config errors", async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith("Error in getUptimeChecksConfigs: Error: denied");
    });
