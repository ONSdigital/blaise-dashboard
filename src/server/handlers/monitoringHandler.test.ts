...
vi.mock("../googleCloudMonitoringApi/monitoring", async (importOriginal) => {
  const original =
    await importOriginal<
      typeof import("../googleCloudMonitoringApi/monitoring.js")
    >();
  return {
    ...original,
...
const getMonitoringUptimeCheckTimeSeriesMock = vi.mocked(
...
describe("Get all uptime checks from API", () => {
  it("should return a 200 status and a json list of 1 items when API returns a 1 item list", async () => {
    ...
    const [googleMonitoringApi] =
      getMonitoringUptimeCheckTimeSeriesMock.mock.calls[0] ?? [];
  ...
  it("should return a clear error when no project ID env var exists", async () => {
    ...
    expect(response.status).toEqual(500);
    expect(response.body).toEqual("Monitoring service is not configured");
