...
describe("Error Logs Handler", () => {
    it("returns non-blaise logs by default", async () => {
        ...
        expect(loggingApi.getErrorLogsLast24Hours).toHaveBeenCalledWith("non-blaise");
    });
    ...
    it("returns Blaise-only logs when scope=blaise", async () => {
        ...
        const response = await supertest(app).get("/api/logs/errors").query({ scope: "blaise" });
    ...
    it("returns NISRA logs when scope=nisra", async () => {
        ...
        const response = await supertest(app).get("/api/logs/errors").query({ scope: "nisra" });
    ...
    it("returns BTS logs when scope=bts", async () => {
        ...
        const response = await supertest(app).get("/api/logs/errors").query({ scope: "bts" });
    ...
    it("returns REST API logs when scope=restapi", async () => {
        ...
        const response = await supertest(app).get("/api/logs/errors").query({ scope: "restapi" });
    ...
    it("returns 500 when provider throws", async () => {
        const loggingApi = {
            getErrorLogsLast24Hours: vi.fn().mockRejectedValue(new Error("permission denied")),
            getCawiSuccessfulLoginCountsLast24Hours: vi.fn().mockResolvedValue([])
        };
        ...
        expect(response.body).toEqual("Failed to get non-blaise GCP error logs: permission denied");
    });
    ...
    it("returns CAWI successful login counts", async () => {
        ...
        const app = express();
        const handler = new ErrorLogsHandler(loggingApi);
        app.get("/api/logs/cawi-logins/success-counts", handler.GetCawiLoginSuccessCounts);

        const response = await supertest(app).get("/api/logs/cawi-logins/success-counts");
    ...
    it("returns 500 when CAWI count provider throws", async () => {
        const loggingApi = {
            getErrorLogsLast24Hours: vi.fn().mockResolvedValue([]),
            getCawiSuccessfulLoginCountsLast24Hours: vi.fn().mockRejectedValue(new Error("denied"))
        };
        ...
        expect(response.statusCode).toEqual(500);
        expect(response.body).toEqual("Failed to get CAWI successful login counts: denied");
    });
    ...
    it("returns clear error when no project ID exists", async () => {
        ...
        const cawiResponse = await supertest(app).get("/api/logs/cawi-logins/success-counts");

        expect(errorResponse.statusCode).toEqual(500);
        expect(errorResponse.body).toEqual("Logging project ID is not configured");
        expect(cawiResponse.statusCode).toEqual(500);
        expect(cawiResponse.body).toEqual("Logging project ID is not configured");
    ...
    it("returns 500 with string reason when CAWI provider rejects non-Error values", async () => {
        const loggingApi = {
            getErrorLogsLast24Hours: vi.fn().mockResolvedValue([]),
            getCawiSuccessfulLoginCountsLast24Hours: vi.fn().mockRejectedValue("denied")
        };
