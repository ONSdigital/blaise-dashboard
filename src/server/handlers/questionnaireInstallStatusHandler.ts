...
export function buildQuestionnaireInstallStatus(
  questionnaire: Questionnaire,
): QuestionnaireInstallStatus {
  const nodes = questionnaire.nodes ?? [];
  const activeNodes = nodes.filter((node) =>
    isNodeActive(node.nodeStatus),
  ).length;
...
export default function questionnaireInstallStatusHandler(
  ...
  return router.get(
    "/api/questionnaires/install-status",
    async (req: Request, res: Response) => {
      try {
        const installStatuses = await getQuestionnaireInstallStatuses(
          blaiseApiClient,
          config,
        );
        return res.status(200).json(installStatuses);
      } catch (error: unknown) {
        logger.error({ error }, "Failed to get questionnaire install status");
        return res
          .status(500)
          .json("Unable to get questionnaire install status.");
      }
    },
  );
}
