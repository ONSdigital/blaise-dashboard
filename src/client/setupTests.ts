...
vi.mock("blaise-design-system-react-components", () => {
  const Header = ({
    title,
    ...props
  }: { title?: React.ReactNode } & Record<string, unknown>) =>
    React.createElement("header", props, title);
  const Footer = ({ children, ...props }: MockComponentProps) =>
    React.createElement("footer", props, children);
  const LoadingPanel = ({
    message,
    children,
    ...props
  }: { message?: React.ReactNode; children?: React.ReactNode } & Record<
    string,
    unknown
  >) => React.createElement("div", props, children ?? message);
  const Panel = ({ children, ...props }: MockComponentProps) =>
    React.createElement("div", props, children);
  const Collapsible = ({
    title,
    children,
    ...props
  }: { title?: React.ReactNode; children?: React.ReactNode } & Record<
    string,
    unknown
  >) =>
    React.createElement(
  ...
  const Table = ({
    ...
    const hasTableSection = childNodes.some(
      (child) =>
        React.isValidElement(child) &&
        ["thead", "tbody", "tfoot", "caption", "colgroup"].includes(
          String(child.type),
        ),
    );
