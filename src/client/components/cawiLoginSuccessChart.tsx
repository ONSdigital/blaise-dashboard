...
export default class CawiLoginSuccessChart extends Component<
  Record<string, never>,
  CawiLoginSuccessChartState
> {
  ...
  renderChart(): ReactElement {
    const maxCount = Math.max(
      ...this.state.data.map((point) => point.count),
      1,
    );
    ...
    const points = this.state.data.map((point, index) => {
      ...
      const y =
        PADDING.top + ((maxCount - point.count) / maxCount) * plotHeight;
      return { x, y, point };
    });
        ...
        {xAxisLabels.map((point, index) => {
          ...
          const x =
            this.state.data.length === 1
              ? PADDING.left + plotWidth / 2
              : PADDING.left +
                (sourceIndex / (this.state.data.length - 1)) * plotWidth;
          return (
            <text
              fill="#3d3d3d"
              fontSize="10"
    ...
    return (
      <>
        <p className="ons-u-mt-s ons-u-mb-s" data-testid="cawi-login-total">
          ...
          <strong>{this.getTotalCount()}</strong>
        </p>
        {this.renderChart()}
      </>
    );
