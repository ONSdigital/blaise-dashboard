...
const getQuestionnaireInstallStatusMock = vi.mocked(
...
describe("App", () => {
  ...
  it("renders correctly", async () => {
    getCaseCompletionReportMock.mockImplementation(() =>
      Promise.resolve(caseCompletionReport),
    );
    getQuestionnairesMock.mockImplementation(() =>
      Promise.resolve(mockQuestionnaireList),
    );
    getMonitoringMock.mockImplementation(() => Promise.resolve([]));
    getBlaiseStatusMock.mockImplementation(() =>
      Promise.resolve([
        { "health check type": "Connection model", status: "OK" },
      ]),
    );
    getQuestionnaireInstallStatusMock.mockImplementation(() =>
      Promise.resolve([
        {
    ...
    getErrorLogsMock.mockImplementation(() =>
      Promise.resolve([
        { timestamp: "2026-06-11T10:00:00.000Z", log: "Example error" },
      ]),
    );
    getCawiLoginSuccessCountsMock.mockImplementation(() =>
      Promise.resolve([
    ...
    const app = renderWithQueryClient(<App />);

    expect(screen.getByText("Getting questionnaires for report")).toBeVisible();
    expect(
      screen.getByText("Getting uptime checks for services"),
    ).toBeVisible();
    expect(await screen.findByText("What is a completed case?")).toBeVisible();
    expect(
      await screen.findByText("Questionnaire install status"),
    ).toBeVisible();
    expect(await screen.findByText("Blaise health check status")).toBeVisible();
    expect(await screen.findByText("Service uptime status")).toBeVisible();
    expect(
      await screen.findByText("Successful CAWI logins (last 24 hours)"),
    ).toBeVisible();
    ...
    expect(
      await screen.findByText("REST API logs (last 24 hours)"),
    ).toBeVisible();
    expect(
      await screen.findByText("Blaise error logs (last 24 hours)"),
    ).toBeVisible();
    expect(
      await screen.findByText("Non-Blaise error logs (last 24 hours)"),
    ).toBeVisible();
    ...
    expect(
      screen.queryByText("Getting questionnaires for report"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Getting uptime checks for services"),
    ).not.toBeInTheDocument();
  ...
  describe("when getting case completion reports errors", () => {
    it("renders an error panel", async () => {
      getQuestionnairesMock.mockImplementation(() =>
        Promise.reject("Cannot get questionnaires"),
      );
      getMonitoringMock.mockImplementation(() => Promise.resolve([]));
      ...
      expect(
        await screen.findByText("Failed to get questionnaires."),
      ).toBeVisible();
    });
  });
  ...
  describe("when no questionnaires are installed", () => {
    it("renders an info panel", async () => {
      ...
      expect(
        await screen.findByText("No questionnaires installed."),
      ).toBeVisible();
    });
  });
    ...
    it("excludes only IPS questionnaires from completed case information", async () => {
      ...
      expect(
        await screen.findByTestId(
          "questionnaire-case-report-questionnaire-OPN2101A",
        ),
      ).toBeVisible();
      expect(
        await screen.findByTestId(
          "questionnaire-case-report-questionnaire-IPA2101A",
        ),
      ).toBeVisible();
      expect(
        screen.queryByTestId(
          "questionnaire-case-report-questionnaire-IPS2201A",
        ),
      ).not.toBeInTheDocument();
  ...
  describe("when no uptime checks are returned or empty response", () => {
    ...
    it("renders uptime checks table when monitoring data is available", async () => {
      getCaseCompletionReportMock.mockImplementation(() =>
        Promise.resolve(caseCompletionReport),
      );
      getQuestionnairesMock.mockImplementation(() => Promise.resolve([]));
      getMonitoringMock.mockImplementation(() =>
        Promise.resolve([
          {
            hostname: "ims-check",
      ...
      expect(await screen.findByTestId("uptimecheck-ims-check")).toBeVisible();
      expect(
        screen.queryByText("No uptime checks data."),
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("when getting Blaise status errors", () => {
    it("renders an info panel with Unable to get Blaise status", async () => {
      ...
      getMonitoringMock.mockImplementation(() => Promise.resolve([]));
      getBlaiseStatusMock.mockImplementation(() =>
        Promise.reject("Cannot get Blaise status"),
      );
      getQuestionnaireInstallStatusMock.mockImplementation(() =>
        Promise.resolve([]),
      );
      ...
      expect(
        await screen.findByText("Unable to get Blaise status"),
      ).toBeVisible();
    });
  });

  describe("when error occurs retrieving uptime checks", () => {
    it("renders an error panel", async () => {
      getCaseCompletionReportMock.mockImplementation(() =>
        Promise.resolve(caseCompletionReport),
      );
      getQuestionnairesMock.mockImplementation(() => Promise.resolve([]));
      getMonitoringMock.mockImplementation(() =>
        Promise.reject("Cannot get uptime checks"),
      );
      getBlaiseStatusMock.mockImplementation(() => Promise.resolve([]));
      getQuestionnaireInstallStatusMock.mockImplementation(() =>
        Promise.resolve([]),
      ...
      expect(
        await screen.findByText("Failed to get uptime checks."),
      ).toBeVisible();
      expect(
        screen.queryByText("No uptime checks data."),
      ).not.toBeInTheDocument();
    ...
    it("renders API error details when available", async () => {
      ...
      expect(
        await screen.findByText(
          "Failed to get monitoring uptimeChecks config data: PERMISSION_DENIED",
        ),
      ).toBeVisible();
      expect(
        screen.queryByText("No uptime checks data."),
